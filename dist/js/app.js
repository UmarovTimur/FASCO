...
*/
    ...
    let IsReduceMotion = !window.matchMedia("(prefers-reduced-motion:reduce)").matches;
    ...
    function roll(targets, vars, reverse) {
        ...
        }), elements = gsapWithCSS.utils.toArray(targets), clones = elements.map((el => {
            let clone = el.cloneNode(true);
            el.parentNode.appendChild(clone);
            return clone;
        })), positionClones = () => elements.forEach(((el, i) => {
            gsapWithCSS.set(clones[i], {
                position: "absolute",
                overwrite: false,
                top: el.offsetTop,
        ...
        elements.forEach(((el, i) => {
            tl.to([ el, clones[i] ], {
                x: reverse ? el.offsetWidth : -el.offsetWidth,
                ...vars,
                y: 0
            }, 0);
        }));
    ...
    var all_gsapWithCSS = gsap.registerPlugin(CSSPlugin) || gsap;
    all_gsapWithCSS.core.Tween;
...
*/    var getOwnPropertySymbols = Object.getOwnPropertySymbols;
    ...
    all_gsapWithCSS.registerPlugin(ScrollTrigger_ScrollTrigger);
    let script_scroll = new locomotive_scroll_esm({
    ...
    window.addEventListener("load", (function(e) {
        window.addEventListener("resize", (e => {
            ...
            script_scroll.update();
        }));
        productsTitlesAnimation();
        timer();
        ScrollTrigger_ScrollTrigger.addEventListener("refresh", (() => script_scroll.update()));
        script_scroll.update();
        if (IsReduceMotion) {
            rollingText();
            socialRolling();
    ...
    function magnetLinks() {
        ...
        magnets.forEach((magnet => {
            if (magnet.hasAttribute("data-strength-text")) {
                ...
                magnet.addEventListener("mouseout", (function(event) {
                    all_gsapWithCSS.to(event.currentTarget, {
                        x: 0,
                        y: 0,
                        duration: 1.5,
                        rotation: "0.001deg",
                    ...
                    all_gsapWithCSS.to(event.currentTarget.querySelector("._magnets-text"), 1.5, {
                        x: 0,
                        y: 0,
                        duration: 1.5,
                        rotation: "0.001deg",
        ...
        function moveMagnet(event) {
            ...
            all_gsapWithCSS.to(magnetButton, {
        ...
        function textMoveMagnet(event) {
            ...
            all_gsapWithCSS.to(magnetText, {
                x: ((event.clientX - boundingText.left) / magnetText.offsetWidth - .5) * strengthText,
                y: ((event.clientY - boundingText.top) / magnetText.offsetHeight - .5) * strengthText,
                rotation: "0.001deg",
                duration: 1.5,
                ease: "elastic"
            });
    ...
    function circleCursor(speed, smoothy) {
        ...
        const tick = () => {
            ...
            const scaleValue = mouseVelocity / smoothy * .5;
    ...
    function rollingText() {
        ...
        document.getElementById("rolling-logos-item");
        let direction = -1;
        let directionName = "down";
        if (!rolling) {
            console.error("Cannot find rollingText");
            return;
        }
        rolling.classList.add("_rolling");
        const roll1 = roll("#rolling-logos #rolling-logos-item", {
            duration: 14
        });
        script_scroll.on("scroll", (self => {
            const SelfDirection = self.direction == "down" ? -1 : 1;
            if (self.direction != directionName) {
                all_gsapWithCSS.to(roll1, {
                    timeScale: direction,
                    overwrite: true
                });
    ...
    function socialRolling() {
        let directionName = "down";
        let gallery = document.getElementById("gallery-rolling"), galleryItem = document.getElementById("gallery-rolling-item");
        if (!gallery) return console.error("Cannot find rollingGallery");
        clone(galleryItem, gallery);
        clone(galleryItem, gallery);
        if (galleryItem.offsetWidth * 1.3 < window.innerWidth) clone(galleryItem, gallery);
        const socialRoll = roll(galleryItem, {
            duration: 10,
            ease: "none"
        }, script_scroll.on("scroll", (self => {
            if (typeof self.currentElements["social__scroll"] === "object") if (directionName != self.direction) {
                let dir = self.direction == "down" ? -1 : 1;
                directionName = self.direction;
                all_gsapWithCSS.to(socialRoll, {
                    timeScale: dir,
                    overwrite: true
                });
            }
        ...
        function roll(element, option, update) {
            ...
            let timeline = all_gsapWithCSS.timeline({
                repeat: -1,
                onReverseComplete() {
                    this.totalTime(this.rawTime() + this.duration() * 10);
                }
    ...
    function productsTitlesAnimation() {
        document.querySelector(".products__catalog-item-current");
        document.querySelector(".products__catalog-mobile-icon");
        document.querySelector(".products__catalog-item._tab-active");
        document.querySelector(".products__catalog");
    }
    function circleFillAnimation() {
        ...
        elements.forEach((element => {
            ...
            let tl = all_gsapWithCSS.timeline();
    ...
    function timer() {
        ...
        function seconds() {
            ...
            setTimeout((() => {
                if (+timer.sec.innerHTML <= 0) {
                    ...
                    if (+timer.sec.innerHTML < 10) timer.sec.innerHTML = `0${timer.sec.innerHTML}`;
                }
                seconds();
            }), "1000  ");
        }
        function minutes() {
            if (+timer.min.innerHTML <= 0) {
                ...
                if (+timer.min.innerHTML < 10) timer.min.innerHTML = `0${timer.min.innerHTML}`;
            }
        }
        function hours() {
            if (+timer.hour.innerHTML <= 0) {
                ...
                if (+timer.hour.innerHTML < 10) timer.hour.innerHTML = `0${timer.hour.innerHTML}`;
            }
        }
        function days() {
            if (+timer.day.innerHTML <= 0) {
                ...
                if (+timer.day.innerHTML < 10) timer.day.innerHTML = `0${timer.day.innerHTML}`;
            }
        }
        seconds();
    }
